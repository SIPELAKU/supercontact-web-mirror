...
export default function CompaniesPage() {
  ...
  const filteredAllCompanies = useMemo(() => {
    return company.filter((item) => {
      ...
      const matchesSearch = searchQuery ? companyName.includes(searchQuery) : true;

      // FILTER BY INDUSTRY
      const matchesIndustry = industry === "all" ? true : (item.industry?.toLowerCase() ?? "") === industry;
      ...
      <div className="mt-6 overflow-auto rounded-lg shadow-lg">
        <div className="flex min-w-[490px] flex-col-reverse lg:flex-row items-center justify-between md:px-4 py-6 gap-5 lg:gap-0">
          <div className="flex flex-col md:flex-row items-center gap-2 md:gap-12">
            {/* Filter by Industry */}
            <FilterByIndustry INDUSTRY_OPTIONS={INDUSTRY_OPTIONS} value={industry} onChange={setIndustry} />

            {/* Filter by Status */}
            <FilterByStatus STATUS_OPTIONS={STATUS_OPTIONS} value={status} onChange={setStatus} />
        ...
        {/* Table */}
        <CompanyTable company={paginatedCompany} isLoading={isLoading} error={error} />
