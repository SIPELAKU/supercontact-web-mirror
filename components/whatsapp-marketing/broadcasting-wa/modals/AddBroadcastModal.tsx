...
import { useCreateBroadcast } from '@/lib/hooks/useBroadcasts';
...
export default function AddBroadcastModal({ open, onClose }: AddBroadcastModalProps) {
  ...
  const validateForm = (action: 'send' | 'draft') => {
    ...
    if (action === 'send') {
      ...
      for (const v of templateVariables) {
        ...
        // Placeholder validation: check for ||Default if it contains {{...}}
  ...
  const handleSubmit = async (action: 'send' | 'draft') => {
    ...
    const payload = {
      ...
      contact_ids: recipientSource === 'recipient' ? selectedContacts.map(c => c.id) : undefined,
      broadcast_group_ids: recipientSource === 'broadcast_group' ? selectedGroups.map(g => g.id) : undefined,
      variables: variables,
    };

    try {
      await createMutation.mutateAsync(payload);
      notify.success(`Broadcast "${name}" ${action === 'send' ? 'queued for sending' : 'saved as draft'}.`);
      handleClose();
    } catch (err: any) {
  ...
  const previewData = useMemo(() => {
    ...
    const replaceVars = (text: string) => {
      ...
      templateVariables.forEach(v => {
        ...
        if (val) {
          // We wrap it in a span-like structure or just replace it?
          // MessagePreview.tsx splits by {{n}}, so if we replace it, it will just show the text.
        ...
        <Grid container spacing={4} sx={{ mt: 0 }}>
          <Grid item xs={12} lg={selectedTemplate ? 7 : 12}>
            <Stack spacing={3}>
                  ...
                  <Box>
                    ...
                    <Box sx={{ mt: 2 }}>
                      <Typography variant="caption" color="text.secondary">
                        Supported dynamic placeholders: <code>&#123;&#123;contact.name||Default&#125;&#125;</code>,
                        <code>&#123;&#123;user.fullname||Default&#125;&#125;</code>, etc.
                        Default value (<code>||Default</code>) is mandatory for dynamic fields.
                      </Typography>
                    </Box>
                  </Box>
                </>
              ...
              {(!templateId && !isLoadingTemplates) && (
                <Alert severity="warning">
                  Please select a template to configure personalization variables. You can still save as a draft without a template.
                </Alert>
              )}
            </Stack>
          </Grid>
