...
export async function registerUser(userData: RegisterData): Promise<RegisterResponse> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      let errorMsg = json.message || json.error?.message || `Registration failed (${res.status})`;
...
export async function verifyOTP(otpData: VerifyOTPData): Promise<VerifyOTPResponse> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      let errorMsg = json.message || json.error?.message || `OTP verification failed (${res.status})`;
      if (json.error?.details && Array.isArray(json.error.details)) {
        const details = json.error.details
          .map((d: any) => `${d.field}: ${d.message}`)
          .join(", ");
