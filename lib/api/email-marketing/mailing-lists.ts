...
export async function fetchMailingListCampaigns(token: string, mailingListId: string, page: number = 1, limit: number = 10): Promise<any> {
  ...
  try {
    ...
    try {
      ...
      logger.error("Failed to parse mailing list campaigns response JSON", {
        status: res.status,
        statusText: res.statusText,
        parseError: parseError.message
      });
    ...
    logger.apiResponse(`/mailing-lists/${mailingListId}/campaigns (GET)`, { status: res.status, response: json });
    ...
    if (!res.ok) {
      logger.error(`Fetch mailing list campaigns failed: ${res.status}`, {
        status: res.status,
        statusText: res.statusText,
        response: json,
        url
      });
      throw new Error(json.error?.message || `Failed to fetch mailing list campaigns (${res.status}: ${res.statusText})`);
    }

    return json;
  } catch (error: any) {
    logger.error("Fetch mailing list campaigns request failed", { error: error.message, url });
    throw error;
  }
}
